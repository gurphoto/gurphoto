require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe Hotel::CitiesController do

  # This should return the minimal set of attributes required to create a valid
  # Hotel::City. As you add validations to Hotel::City, be sure to
  # update the return value of this method accordingly.
  def valid_attributes
    {}
  end
  
  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # Hotel::CitiesController. Be sure to keep this updated too.
  def valid_session
    {}
  end

  describe "GET index" do
    it "assigns all hotel_cities as @hotel_cities" do
      city = Hotel::City.create! valid_attributes
      get :index, {}, valid_session
      assigns(:hotel_cities).should eq([city])
    end
  end

  describe "GET show" do
    it "assigns the requested city as @city" do
      city = Hotel::City.create! valid_attributes
      get :show, {:id => city.to_param}, valid_session
      assigns(:city).should eq(city)
    end
  end

  describe "GET new" do
    it "assigns a new city as @city" do
      get :new, {}, valid_session
      assigns(:city).should be_a_new(Hotel::City)
    end
  end

  describe "GET edit" do
    it "assigns the requested city as @city" do
      city = Hotel::City.create! valid_attributes
      get :edit, {:id => city.to_param}, valid_session
      assigns(:city).should eq(city)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Hotel::City" do
        expect {
          post :create, {:city => valid_attributes}, valid_session
        }.to change(Hotel::City, :count).by(1)
      end

      it "assigns a newly created city as @city" do
        post :create, {:city => valid_attributes}, valid_session
        assigns(:city).should be_a(Hotel::City)
        assigns(:city).should be_persisted
      end

      it "redirects to the created city" do
        post :create, {:city => valid_attributes}, valid_session
        response.should redirect_to(Hotel::City.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved city as @city" do
        # Trigger the behavior that occurs when invalid params are submitted
        Hotel::City.any_instance.stub(:save).and_return(false)
        post :create, {:city => {}}, valid_session
        assigns(:city).should be_a_new(Hotel::City)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Hotel::City.any_instance.stub(:save).and_return(false)
        post :create, {:city => {}}, valid_session
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested city" do
        city = Hotel::City.create! valid_attributes
        # Assuming there are no other hotel_cities in the database, this
        # specifies that the Hotel::City created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        Hotel::City.any_instance.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, {:id => city.to_param, :city => {'these' => 'params'}}, valid_session
      end

      it "assigns the requested city as @city" do
        city = Hotel::City.create! valid_attributes
        put :update, {:id => city.to_param, :city => valid_attributes}, valid_session
        assigns(:city).should eq(city)
      end

      it "redirects to the city" do
        city = Hotel::City.create! valid_attributes
        put :update, {:id => city.to_param, :city => valid_attributes}, valid_session
        response.should redirect_to(city)
      end
    end

    describe "with invalid params" do
      it "assigns the city as @city" do
        city = Hotel::City.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Hotel::City.any_instance.stub(:save).and_return(false)
        put :update, {:id => city.to_param, :city => {}}, valid_session
        assigns(:city).should eq(city)
      end

      it "re-renders the 'edit' template" do
        city = Hotel::City.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Hotel::City.any_instance.stub(:save).and_return(false)
        put :update, {:id => city.to_param, :city => {}}, valid_session
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested city" do
      city = Hotel::City.create! valid_attributes
      expect {
        delete :destroy, {:id => city.to_param}, valid_session
      }.to change(Hotel::City, :count).by(-1)
    end

    it "redirects to the hotel_cities list" do
      city = Hotel::City.create! valid_attributes
      delete :destroy, {:id => city.to_param}, valid_session
      response.should redirect_to(hotel_cities_url)
    end
  end

end
